...
function displayCategoryItems(categoryName, items, container, activeFilter = null) {
  ...
  items.forEach((item, index) => {
    ...
    // Extract item name and grams using pattern matching
    const itemText = item.ITEM;
    const gramsMatch = itemText.match(/\s*[\d.]+\s*oz\s*\((\d+)\s*g\)\s*$/);
    const itemName = gramsMatch ? itemText.replace(/\s*[\d.]+\s*oz\s*\(\d+\s*g\)\s*$/, '') : itemText;
